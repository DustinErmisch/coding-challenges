 ...
 * 
 * Create a function that will remove any repeated character(s) in a word passed
 * to the function. Not just consecutive characters, but characters repeating
 * anywhere in the string. Examples
 * 
 ...
 * Notes
 * 
 * No more than two words will be passed. Notice that a string is iterable.
 */
public class RemoveRepeatedCharacters {
	...
	public static String unrepeated(String word) {
		...
		for (int i = 0; i < wordArray.length; i++) {
			...
			for (int j = wordArray.length - 1; j > i; j--) {
				...
				if (wordArray[i].equals(wordArray[j])) {
					// dann das Element mit wohl nie als Parameter vorkommender Zeichenfolge
					// ueberschreiben
					wordArray[j] = "+%+***";
		...
		for (int i = 0; i < wordArray.length; i++) {
			// und nur wenn das Element nicht gleich der wohl nie als Parameter vorkommender
			// Zeichenfolge ist
